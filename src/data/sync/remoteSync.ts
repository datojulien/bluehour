...
export interface RemoteSnapshotForSync {
...
export function planRemoteSnapshotSync(
  local: BluehourSnapshot,
  remote: RemoteSnapshotForSync,
  {
    supportedSchemaVersion = BLUEHOUR_SCHEMA_VERSION,
    remoteLabel = "Remote profile"
  }: {
  ...
  if ((remote.schemaVersion ?? 1) > supportedSchemaVersion) {
    return statePlan(
      "read_only_recovery",
      ...
      [],
      false,
      `${remoteLabel} schema ${remote.schemaVersion} is newer than this Bluehour build supports.`
    );
  }
  ...
  if (remote.remoteRevision === 0) {
    return statePlan("push_local", remote.remoteRevision, nextRemoteRevision, [], [], true, `${remoteLabel} is empty; local data should be pushed.`);
  }

  if (remote.remoteRevision < localRemoteRevision) {
    return statePlan("push_local", remote.remoteRevision, nextRemoteRevision, [], [], true, "Local revision is ahead of the remote profile.");
  }
  ...
  if (remote.remoteRevision === localRemoteRevision) {
    ...
    return statePlan("no_op", remote.remoteRevision, localRemoteRevision, [], [], false, "Local data and remote profile are in sync.");
  }
