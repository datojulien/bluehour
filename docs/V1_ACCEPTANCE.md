...
Bluehour is accepted as `1.0.0-rc.4` when the automated gate passes and the manual stable-release gate remains clearly documented as incomplete.
...
- [x] Profile Health detects and repairs the interrupted first-cycle state without deleting data, and keeps multiple-open-cycle or unsupported remote states in read-only recovery.
...
- [x] Google Drive app-data vault remains the primary sync store; optional Google Sheets export is inspection only and is not expected after sign-in.
...
- [ ] Profile Health repair and hidden Drive vault reset verified with a real Google account.
