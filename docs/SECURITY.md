...
- Continue-with-Google discovery uses Drive file metadata for app-accessible spreadsheets only and keeps the same `drive.file` OAuth scope.
...
- Continue-with-Google inspection does not write to the remote Sheet or local profile until the user confirms restore/replacement.
