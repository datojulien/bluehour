...
- Gemini first-cycle setup and cycle reports require explicit live-profile approval before sending redacted profile data to Gemini. The setup helper sends planned salary, account labels, obligations, categories, and budget preferences; the cycle report sends salary-cycle transactions, plans, subscriptions, goals, and budget state. The Gemini API key is entered per request and is not written to IndexedDB, localStorage, sessionStorage, URLs, Drive vault files, optional Sheets, logs, or repository files. Gemini results are transient in the browser; selected budget proposals only change records after the user accepts the setup template or completes salary-cycle close.
- Gemini first-cycle setup and cycle reports are educational budgeting guidance only. They must not be represented as investment, tax, legal, credit, insurance, or regulated financial advice.
